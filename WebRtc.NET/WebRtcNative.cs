...
namespace WebRtc.NET
{
    public class WebRtcNative : IDisposable
    {
        ...
        static WebRtcNative()
        {
            var dir = Path.Combine(Path.Combine(Path.GetTempPath(), IntPtr.Size == 8 ? "x64" : "x64"), version);
            ...
            if (!File.Exists(file))
            {
                File.WriteAllBytes(file, IntPtr.Size == 4 ? Resources.WebRtcNative_x64 : Resources.WebRtcNative_x86);
            }
        ...
        {
            Debug.WriteLine(String.Format("_OnDataMessage: {0}", msg));
            OnDataMessage(msg);
        }
